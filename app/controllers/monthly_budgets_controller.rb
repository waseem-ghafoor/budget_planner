class MonthlyBudgetsController < ApplicationController
  ...
  def all_financial_years # rubocop:disable Metrics/AbcSize
    ...
    first_year = first.month <= @user.user_profile.starting_month ? first.year - 1 : first.year
    last_year  = last.month > @user.user_profile.starting_month ? last.year : last.year - 1

    min_first_year = Time.zone.today.year - @user.user_plan[:history]
    first_year = min_first_year > first_year ? min_first_year : first_year
  ...
  def yearly_budgets
    yearly_budgets = @user.monthly_budgets.of_the_user_year(
      params[:financial_year].to_i, @user
    )
