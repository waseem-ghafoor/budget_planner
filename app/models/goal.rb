class Goal
  ...
  scope :during_user_year, lambda { |year, user|
    where(
      :end_date.gte => Date.new(year, user.user_profile.starting_month + 1, 1),
      :start_date.lte => Date.new(year + 1, user.user_profile.starting_month, 31)
    )
  }
