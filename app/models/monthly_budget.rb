class MonthlyBudget
  ...
  validate :user_access_permission_history
  ...
  scope :of_the_user_year, lambda { |year, user|
    d = Date.new(year, user.user_profile.starting_month + 1, 1)
    where(
      :month.gte => d,
      :month.lte => d + 12.months - 1.day
    )
  }
  ...
  def user_access_permission
    validity = user.user_plan[:plan_upto_in_years]
    return if (month.year - current_user_year) < validity

    errors.add(:month, "your plan doesn't support planning further")
  end

  def user_access_permission_history
    history = user.user_plan[:history]
    return if (current_user_year - month.year) < history

    errors.add(:month, "your plan doesn't support planning further history")
  end
  ...
  def current_user_year
    date = Time.zone.today
    date.month < (user.user_profile.starting_month + 1) ? date.year - 1 : date.year
  end
end
